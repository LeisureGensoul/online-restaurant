...
import com.whu.order.domain.coupon.vo.UserCouponVO;
...
import com.whu.order.mini.payment.WeChatPayment;
...
@RestController
@RequestMapping(value = "/onlinerestaurant/app/user")
@Api(tags = "User操作接口")
@Slf4j
@Validated
public class UserController {
    ...
    public ActionResult<PayInfo> prePay(@Valid @RequestBody PrePayQuery prePayQuery, @RequestHeader("token") String token) {
        LoginUser loginValue = redisService.getLoginValue(token);
        Order order = paymentService.creatonlinerestaurant(prePayQuery, loginValue.getId());

        //调用微信支付mock接口
        PayInfo payInfo = WeChatPayment.pronlinerestaurant(order);
