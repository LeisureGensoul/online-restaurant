package com.whu.order.controller.user;

import com.whu.order.commons.result.ActionResult;
import com.whu.order.commons.utils.JWTUtils;
import com.whu.order.domain.user.dto.WxUserDTO;
import com.whu.order.domain.user.vo.LoginVO;
import com.whu.order.domain.user.vo.UserVO;
...
import com.whu.order.types.user.WxLoginQuery;
import com.whu.order.types.user.WxUserProfile;
import com.whu.order.mini.user.service.LoginService;
...
@Api(tags = "User登录接口")
@RestController
@RequestMapping("/onlinerestaurant/app/user")
@Slf4j
@Validated
public class LoginController {
    ...
    public ActionResult<Object> insertWxUser(@RequestBody WxUserProfile user, @RequestHeader("token") String token) {
        ...
        LoginUser loginUser = redisService.getSessionKeyAndOpenId(token);
