...
export default {
  ...
  created() {
   ...
   computed: {
    filteredDishes() {
      return this.dishes.filter(item => item.wait_num !== 0);
    }
  },
  mounted() {
    this.fetchdishes();
    this.interval = setInterval(this.fetchdishes, 5000); // 每5秒获取一次数据
  },
  beforeDestroy() {
   ...
   methods: {
    fetchdishes() {
      // 后端请求
      instance.get('/api/recommendation')
        .then(response => {
          console.log("服务器返回的数据:", response.data.dishes);
          this.dishes = response.data.dishes;
          this.ongoingTasks = this.dishes.reduce((acc, item) => acc + item.wait_num, 0);
 ...
 '#FF7500', // red to Yellow
  ...
  '#FF4500'  // Orange-Red 2
    ...
    handletaskcardClick(item) {
      ...
      instance.delete('/api/dish', {
        ...
        .then(response=> {
          ...
          item.wait_num-=1;
          if(item.wait_num==0){
            this.dishes = this.dishes.filter(e => e.dish_name !== item.dish_name);
          }
          this.totalTasks=this.ongoingTasks+this.completedTasks;
    ...
    initializeCompletedTasks() {
      ...
      if (storedDate !== today) {
        ...
        this.completedTasks = parseInt(localStorage.getItem('completedTasks') || '0');
      }
    },
    scheduleDailyReset() {
      const now = new Date();
  ...
  watch: {
    completedTasks(newVal) {
      localStorage.setItem('completedTasks', newVal);
    }
  }
};
</script>
